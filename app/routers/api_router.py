...
none_auth_resp = JSONResponse(status_code=401, content={"code": 401, "data": "未授权的访问"})
...
async def post_email_route(email: email_model.Email, authorization: str = Header(None)):
    ...
    if mailbox_id is not None:
        return JSONResponse(status_code=200,
                            content={"code": 200, "data": {"mailbox_id": mailbox_id, "message": "邮件已加入队列"}})
    else:
        return JSONResponse(status_code=400, content={"code": 400, "data": "邮件加入队列失败"})


@router.get("/email/{mailbox_id}", response_model=email_model.EmailRespond, tags=["email"])
async def get_email_route(mailbox_id: int, authorization: str = Header(None)):
    ...
    if len(record) == 0:
        return JSONResponse(status_code=404, content={"code": 404, "data": "邮件不存在"})
    else:
        return JSONResponse(status_code=200, content={"code": 200, "data": jsonable_encoder(record)})


@router.delete("/email/{mailbox_id}", response_model=email_model.EmailRespond, tags=["email"])
async def delete_email_route(mailbox_id: int, authorization: str = Header(None)):
    ...
    if row_count == 1:
        return JSONResponse(status_code=200,
                            content={"code": 200, "data": {"mailbox_id": mailbox_id, "message": "计划发送已取消"}})
    else:
        return JSONResponse(status_code=400, content={"code": 400, "data": "计划发送取消失败，邮件可能已发送"})
